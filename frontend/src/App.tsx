...
function App() {
  ...
  const [selectedBlock, setSelectedBlock] = useState<BlockComparison | null>(
    null
  );
    ...
    return (
      <div className="text-sm">
        ...
        {details.isSandwichTarget && (
          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 ml-2">
            Sandwich Target
          </span>
        )}
      </div>
            ...
            <table className="w-full">
              ...
              <tbody className="font-sans">
                {comparisons.slice(0, 10).map((comparison) => (
                  <tr
                    key={comparison.id}
                    onClick={() => setSelectedBlock(comparison)}
